...
use sc2::structs::{Details, Player, Color, BNet};
...
fn main() -> Result<(), Box<dyn std::error::Error + 'static>> {
    ...
    let details =  sc2::parser::parse_details(typed_expect!(parsed, sc2::structs::DataType::HashMap));
    println!("{:#?}", details);

    let file_data = find_and_read_file(data, String::from("replay.game.events"), &hashtable,
                                       &crypttable, &blocktable, &mpqheader).unwrap();
    let events = sc2::parser::parse_events(data, sc2header.game_version.build.abs() as u64).unwrap();
